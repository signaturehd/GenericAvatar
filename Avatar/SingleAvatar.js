...
class SingleAvatar extends Component {
  ...
  convertInitial (name) {
    ...
    let first = words[0].charAt(0) +''+words[words.length - 1].charAt(0)
    let splitUserInitial = first.toUpperCase()
  ...
  checkAvatarLength (avatarDefaultColor) {
    if(avatarDefaultColor) {
      if(this.isDataURL(this.props.backgroundColor)) {
        return ''
      } else {
        const count = this.props.initials.length
        return count === 1 ? '+' + this.props.initials : this.convertInitial(this.props.initials)
      }
    } else {
      if(this.isDataURL(this.props.backgroundColor)) {
        return ''
      } else {
        return this.convertInitial(this.props.initials)
      }
    }
  }
  ...
  render () {
    ...
    return (
      <h4
        ...
        >{ this.checkAvatarLength(defaultColor) }</h4>
    )
  }
}
